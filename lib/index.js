...
var spotifier = (function (self) {
	...
	self.lookupAlbum = function (options, albumId, callback) {
		if (typeof callback === 'undefined' && typeof albumId === 'function') {
			callback = albumId;
			albumId = options;
			options = {};
		}
		...
		if (!albumId) {
			return setImmediate(
				callback,
				new Error('albumId must supplied'));
		}
		...
		return ensureAccessToken(authorization, function (err, token) {
			...
			return request({
				...
				uri : [self.options.albumUrl, albumId].join('/')
			}, function (err, res, body) {
				if (err) {
					return callback(err);
				}
